...
本插件在 root slot 中组合侧边栏、主内容和右栏。侧边栏宽度为 264～420px，默认为 280px，收起后保留 56px 控制栏；窗口宽度低于 1024px 时，展开的侧边栏会在全宽主内容上显示为带遮罩的覆盖抽屉，打开右侧面板或切换当前会话会将其关闭，但不会改变宽屏宽度偏好。右侧面板首次打开时使用视口宽度的 45%，之后保留用户的像素宽度偏好，上限为 70%。为给中栏保留 400px，框架先将右侧面板缩减至 300px，再报告空间不足，使占用方将其关闭，最后才进一步压缩中栏。拖动没有过渡延迟；调整大小手柄只在宽屏显示，右侧手柄在关闭或全屏时不显示。
...
在自动收起断点以下，AppFrame 将侧边栏网格轨道设为零，同时把侧边栏列定位在主内容上方。点击遮罩、切换侧边栏、切换会话或切换主面板都会调用 `closeNarrowSidebar`；该动作只清除 `narrowExpanded`，因此窗口变宽后仍会保留用户的宽屏宽度。
