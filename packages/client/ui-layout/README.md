...
The root slot composes the sidebar, main content, and right column. The sidebar spans 264–420px, defaults to 280px, and retains a 56px rail when collapsed; below 1024px an expanded sidebar becomes an overlay drawer over the full-width center with a scrim, while opening the right panel or changing the current Session dismisses it without changing the wide-screen width preference. The right panel first opens at 45% of the viewport, then retains the user's pixel preference, capped at 70%. To protect 400px for the center, the frame first reduces the right panel to 300px, then reports insufficient room so its occupant closes it, and only then compresses the center further. Dragging has no transition delay; resize handles are desktop-only and the right handle is absent while closed or fullscreen.
...
Below the auto-collapse breakpoint, AppFrame sets the sidebar grid track to zero while the sidebar column is positioned over the center. Scrim clicks, sidebar toggles, Session changes, and main-panel changes call `closeNarrowSidebar`; that action only clears `narrowExpanded`, so the user's wide-screen width remains available after widening.
