...
| 11 | Approvals are observed, never answered, by the shell: mux WebSocket `approval/requested` frames (replayed with stable ids on connect) drive notifications/badge; answers stay with the web UI via POST /api/respond | The host owns approval settlement and audit correlation; a second answerer would race clients and corrupt the pending registry | 2026-08-23 |
| 12 | Embedded runtime snapshot resolves LAST (env → default → walk-up → `Resources/runtime`) | An app kept inside the checkout must keep developing against live sources; only a relocated bundle needs self-hosting. NodeLocator likewise prefers the snapshot's `.node-bin/node` once that root wins | 2026-08-23 |
| 13 | Mobile access = t3code model: token gate in front of the loopback server, never an unauthenticated LAN bind, never a relay cloud | The harness fence is explicitly not auth ("authentication stay[s] out of scope"); a gate process adds pairing (one-time code → cookie) while the fence keeps its confused-deputy defenses. User picked direct-LAN over bb's Cloudflare tunnel | 2026-08-23 |
